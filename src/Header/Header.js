import React from 'react'
import logo from '../assest/Logo_final_no_bg.png'
import '../Header/Header.css'

function Header() {
    return (
        <div>
            <header className="invoice-header">
                <div className='header-content'>
                    <div className='logo_container'>
                       
                    </div>
                    <div className='header-left'>
                        <center> <h4 className='tax_invoice'><u>TAX INVOICE</u></h4>
                            <p className='Lakshmi'>LAKSHMI PUMP INDUSTRIES</p>
                            </center>
                        <p className='address' sx="font-size: larger;">420/2,V.K.Road,Thanneer Panthal,Peelamedu, Coimbatore - 641 004</p></div>
                    <div className='header_right'>
                        <p><>GSTIN/UIN:</><b>33AAMFL7965M1ZJ</b></p>
                        <p>PAN:<b> AAMFL7965M</b></p>
                        <p>Mobile:<b> 9786627043</b></p>
                        <p>E-Mail:<b>lpipumps@gmail.com</b></p>
                    </div>
                </div>

            </header></div>
    )
}

export default Header